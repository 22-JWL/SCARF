...
import old.rag_pipeline_old as rag_pipeline_old
...
output_model = api.model('OutputModel', {
    ...
    'is_ambiguous': fields.Boolean(description='Stage2 모호성 판단 결과'),
    ...
    'slots':        fields.Raw(description='슬롯 필링 현재 상태'),  # ← 추가
...
class Instruct(Resource):
    ...
    def post(self):
        ...
        if session_id and answer is not None:
            ...
            rag_result = rag_pipeline_old.continue_session(session_id, answer)
        ...
        rag_result = rag_pipeline_old.process_new_query(user_input)
