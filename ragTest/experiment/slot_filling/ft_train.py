...
CFG = {
    ...
    "data_dir": "",           # 파인튜닝 데이터 경로
    # "data_dir": "data",                  # [원본] 전체 학습 시
    "output_dir": "checkpoints_finetune",  # 파인튜닝 체크포인트 저장 경로
    # "output_dir": "checkpoints",         # [원본]
    "checkpoint_path": "checkpoints/slot_filling/best_model.pt",      # 로드할 체크포인트
    "candidate_vocab_path": "checkpoints/slot_filling/candidate_vocab.json",  # 기존 vocab
    "max_length": 128,
    "batch_size": 32,
    #"lr": 5e-6,        # 파인튜닝용 낮은 lr
    "lr": 3e-6,  # 지피티 추천 설정
...
def train():
    ...
    # ── 기존 candidate_vocab 로드 (파인튜닝 시 vocab 유지) ────────────────
    print(f"candidate_vocab 로드: {CFG['candidate_vocab_path']}")
    with open(CFG["candidate_vocab_path"], "r", encoding="utf-8") as f:
        pretrained_vocab = json.load(f)

    # ── 데이터셋 ──────────────────────────────────────────────────────────
    train_ds = SlotFillingDataset(
        f"{CFG['data_dir']}finetune_data.jsonl", CFG["model_name"], CFG["max_length"]
    )
    val_ds = SlotFillingDataset(
        f"{CFG['data_dir']}val.jsonl", CFG["model_name"], CFG["max_length"]
    )

    # 파인튜닝: 기존 vocab으로 덮어씌워서 candidate 인덱스 일관성 유지
    train_ds.candidate_vocab = pretrained_vocab
    val_ds.candidate_vocab = pretrained_vocab
    ...
    num_candidates = len(pretrained_vocab)
    print(f"Candidate vocab size (pretrained): {num_candidates}")

    # 파인튜닝용 vocab도 저장
    with open(f"{CFG['output_dir']}/candidate_vocab.json", "w", encoding="utf-8") as f:
        json.dump(pretrained_vocab, f, ensure_ascii=False, indent=2)
    ...
    # ── 체크포인트 로드 ────────────────────────────────────────────────────
    print(f"체크포인트 로드: {CFG['checkpoint_path']}")
    ...
    for epoch in range(1, CFG["epochs"] + 1):
        ...
        val_loss, status_f1, cat_acc = evaluate(
            model, val_loader, device, pretrained_vocab
        )
    ...
    print("파인튜닝 완료!")
    print(f"best_model → {CFG['output_dir']}/best_model.pt")
